...
- [x] Write failing tests proving a completed run emits exactly one version-1
  snapshot with participants, scores, opaque context, and symbol-only boards.
- [x] Prove abandoned and replaced runs do not emit completion records.
- [x] Add the immutable DTO builder and optional completion sink.
- [x] Run server coordinator tests and commit the green task.
...
- [x] Write failing repository/recorder tests for idempotent insertion, participant
...
- [x] Add normalized record, participant, and bookmark tables plus indexes.
- [x] Implement repository queries and the bounded asynchronous recorder.
- [x] Run unit tests and PostgreSQL 16 integration tests; commit the green task.

### Task 3: Authenticated history API

**Files:** `server/src/matchHistory/http.ts`, `server/src/app.ts`, `server/src/server.ts`, app tests.

- [x] Write failing HTTP tests for authentication, perspective projection,
  cursor/filter handling, participant-only detail, bookmark origin checks,
  no-store responses, and error mapping.
- [x] Add `GET /api/history`, `GET /api/history/:matchId`, and
  `PUT /api/history/:matchId/bookmark`.
- [x] Wire the repository and recorder through startup and graceful shutdown.
- [x] Run the server suite and commit the green task.

### Task 4: Web data boundary

**Files:** `web/src/history/historyClient.ts`, `web/src/history/types.ts`, tests.

- [x] Write failing tests for list/detail decoding, bookmark requests, unknown
  symbols, 401 session expiry, and retryable failures.
- [x] Implement the dedicated client and mechanics-independent DTOs.
- [x] Run focused web tests and commit the green task.
...
- [x] Write failing component and view-model tests for profile navigation, return
...
- [x] Implement the full history screen using the existing brush/block visual
  system, ≥48 px controls, semantic focus states, and responsive paired boards.
- [x] Verify unknown symbol fallback and reduced-motion behaviour.
- [x] Run web tests, lint, and build; commit the green task.

### Task 6: Documentation and complete verification

**Files:** `docs/ROADMAP.md`, `DESIGN.md`.

- [x] Record durable static history separately from deferred chess-style replay
  and the future admin research/cleanup console.
- [x] Run `npm test`, `npm run lint`, `npm run build`, PostgreSQL integration
  tests, and `docker build` from the repository root.
- [x] Drive a real two-account match plus a third-account denial in the browser;
  verify perspective, boards, totals, pagination, and independent bookmarks.
- [x] Review the final diff for secrets, raw history logging, unrelated UI edits,
